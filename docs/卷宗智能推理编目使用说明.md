...
单文件智能体模式适合需要保留执行轨迹、低置信复核和最小恢复摘要的编目任务。该模式使用 OpenAI-compatible HTTP API 配置和旧 `z-ai` fallback，与普通单文件预测共用同一套提取、视觉分析、文本分析和候选目录规则。
...
| --- | --- |
| `--agent` | 启用单文件智能体状态机。 |
| `--trace <path>` | 将每个执行步骤写入 JSONL 轨迹文件；未提供时默认写入程序目录下的 `agent_trace.jsonl`。 |
| `--review-output <path>` | 当状态为 `NEEDS_REVIEW` 或 `FAILED` 时写入人工复核 JSON。 |
| `--resume <trace>` | 读取已有 trace 的最后状态并输出恢复摘要；第二阶段不会从中间步骤继续执行工具。 |
...
- 不支持 batch agent mode；批量处理仍使用普通 `--batch`。
- 不支持 partial resume；最后状态不是 `COMPLETED`、`NEEDS_REVIEW` 或 `FAILED` 时会返回明确失败原因。
- 还没有工具级 retry policy；失败会进入 `FAILED` 并依赖复核包排查。
