...
批量模式只处理指定目录下的直接文件，不递归子目录。`--trace` 在批量智能体模式下会作为运行目录使用，运行完成后生成：
...
| `--review-output <path>` | 当状态为 `NEEDS_REVIEW` 或 `FAILED` 时写入人工复核 JSON。 |
...
- 工具调用接入 retry policy，默认保持一次尝试；测试和后续调用可配置重试次数、退避间隔和可重试错误。
