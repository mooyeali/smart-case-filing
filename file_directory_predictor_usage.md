...
脚本优先通过 OpenAI-compatible HTTP API 调用模型。至少需要配置 `base_url`、`api_key`、`model` 三个参数。
...
```text
chat   : AI_CHAT_BASE_URL / AI_CHAT_API_KEY / AI_CHAT_MODEL，缺省回退 AI_BASE_URL / AI_API_KEY / AI_MODEL
vision : AI_VISION_BASE_URL / AI_VISION_API_KEY / AI_VISION_MODEL，缺省回退 AI_BASE_URL / AI_API_KEY / AI_MODEL
```
...
如果未配置完整的 HTTP 三件套，脚本会回退到旧的 `z-ai` CLI：
...
使用 fallback 前需要确认 `z-ai` 已安装、已登录或已配置可用凭据，并且以下命令能正常返回：
...
`VLMAnalyzer.analyze()` 对图片或 PDF 渲染页调用 `_run_zai_vision()`。该函数优先使用 `AI_VISION_*` / `AI_*` 配置的 OpenAI-compatible HTTP API；未配置完整三件套时回退：
...
`LLMAnalyzer.analyze_text()` 对提取文本调用 `_run_zai_chat()`。该函数优先使用 `AI_CHAT_*` / `AI_*` 配置的 OpenAI-compatible HTTP API；未配置完整三件套时回退：
...
脚本把文件画像和候选规则交给 `_run_zai_chat(..., thinking=True)`，要求模型从候选编号中选择最匹配的一条。配置了 HTTP 三件套时使用 OpenAI-compatible API；否则回退到 `z-ai chat -t`。解析结果时优先相信 `selected_index`，然后用候选规则反写案件类型、卷宗、二级目录、材料类别和编目示例，避免模型自由生成不存在的规则字段。
...
也要确认模型服务鉴权、网络和超时时间是否正常。脚本默认每次模型调用超时为 180 秒。
