...
- 可验收批量智能体模式支持执行轨迹、run manifest、低置信复核、partial resume、retry CLI、模型配置预检、人工复核决定记录、run 审计报告、无模型全链路验收命令和安全归档计划。
...
只有 `COMPLETED` 且非 `low` 置信度的文件会进入可执行计划；`NEEDS_REVIEW`、`FAILED`、低置信和字段不完整的文件会被标记为 `blocked`。
